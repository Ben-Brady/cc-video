...
def calculate_permonitor(display: monitor.MonitorDisplay) -> tuple[int, int]:
...
def encode_numpy_frame(
    ...
    if frame.shape[0] != newSize[0] or frame.shape[1] != newSize[1]:
...
def encode_pillow_frame(
    display: monitor.MonitorDisplay, img: Image.Image, executor: ThreadPoolExecutor
) -> str:
    displaySize = calculate_display_size(display)
    newSize, offset = calculate_resize_parameters(
...
def calculate_resize_parameters(
...
def encode_frame(
    ...
    args = [(monitor, display) for monitor in monitors]
    return "".join(executor.map(encode_monitor, args))


def encode_monitor(arg: tuple[Image.Image, monitor.MonitorDisplay]):
    img, display = arg
    output = ""
    ...
    palette = img.palette
    if not palette:
        raise Exception("Image not quantized")
