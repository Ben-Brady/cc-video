...
class Sourcer:
    ...
    def __init__(
        ...
        **sourcer_params,
        ...
        if isinstance(source_demuxer, str):
    ...
    def retrieve_metadata(self, pretty_json=False, force_retrieve_missing=False):
        ...
        **Returns:** `metadata` or `(metadata, metadata_missing)`, formatted as JSON string or python dictionary.
        ...
        # present in metadata.
        if isinstance(self.__source, str) and self.__source_demuxer is None:
    ...
    def __validate_source(self, source, source_demuxer=None, forced_validate=False):
        ...
        **Returns:** `True` if passed tests else `False`.
        ...
        is_stream = isinstance(source, io.BytesIO)
        if is_stream:
            source_src = "-"
        else:
            source_src = source
        ...
        metadata = (
            check_sp_output(
                meta_cmd,
                stdin=source if is_stream else sp.DEVNULL,
