...
class FFdecoder:
    ...
    def formulate(self, stream: io.BytesIO):
        """
        ...
        **Returns:** A reference to the FFdecoder class object.
        ...
        if self.__initializing:
            ...
            self.__launch_FFdecoderline(stream, input_params, output_params)
    ...
    def __launch_FFdecoderline(self, stdin: io.BytesIO, input_params, output_params):
        """
        ...
        cmd = (
            ...
            + (
                ...
                if (
                    "source_demuxer" in self.__sourcer_metadata.keys()
                    and self.__sourcer_metadata["source_demuxer"]
                )
                else []
            )
            + ["-i", "-"]
        ...
        if self.__verbose_logs:
            ...
            self.__process = sp.Popen(
                cmd, stdin=stdin, stdout=sp.PIPE, stderr=None
            )
        else:
            print(cmd)
            # In silent mode
